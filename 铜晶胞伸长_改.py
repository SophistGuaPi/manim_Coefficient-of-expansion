...
class View1_0(Scene):
    def construct(self):
        ...
        axes2 = Axes(x_range=[-3,0.3],y_range=[0,4],x_axis_config={"include_tip":True},y_axis_config={"include_tip":True}).scale(0.3).to_corner(UR).shift(DOWN*0.4+RIGHT*1.5)
        axes2_label=axes2.get_axis_labels(r"E",r"x",).shift(RIGHT*1.5+UP*0.4).scale(0.7)
        axes2_label[0].shift(DOWN*0.3+RIGHT*0.15).scale(0.7)
        axes2_label[1].shift(LEFT*0.2+UP*0.3).scale(0.7)

        d6=SmallDot(axes2.c2p(h.get_value(),axes0.p2c(d2.get_center())[0]-0.4))
        ...
        d6.add_updater(lambda d:d.move_arc_center_to(axes2.c2p(h.get_value(),axes0.p2c(d2.get_center())[0]-0.4)))
        ...
        self.play(c1.animate.move_to(axes0.c2p((solute0.get_value()+solute1.get_value())/2
            +(solute0.get_value()-solute1.get_value())/2*npy.cos(((h.get_value()-2)*20)*h.get_value()+1.8)
                                                    ,0)), run_time=0.1)
        self.play(h.animate.set_value(E_end),solute0.animate.set_value(h.get_value()),solute1.animate.set_value(h.get_value()),rate_func=linear,run_time=1)
        self.wait()

        self.play(camera.animate.scale(0.5, about_point=axes2.get_corner(UR)))
        self.wait()

        tex0 = Tex(r"E=\varepsilon_{k}+U", font_size=30).next_to(camera.get_corner(UL), DR)
        tex1 = Tex(r"\overline{\varepsilon_{k}}=k_{B}T", font_size=30).next_to(tex0, DOWN)
        tex=VGroup(tex0,tex1,axes2_label[0])
        self.play(Write(tex0))
        self.play(Write(tex1))
        d6.suspend_updating()
        tex.lock_data(r"T")
        self.play(TransformMatchingShapes(tex,Tex(r"T").match_width(axes2_label[0]).move_to(axes2_label[0])),
                  axes2.y_axis.animate.shift(LEFT*3),axes2_label[1].animate.shift(LEFT*3))

        path_dot=[]
        path_dot_index=[]
        slope_list=[]
        path_dot_group=VGroup()
        cut_lenth = int(len(path0.get_all_points()) / 5)
        real_lenth = path0.get_all_points() - 2 * cut_lenth
        for i in range(5):
            print(f"lenth{i}:",path0.get_all_points()[i],int(cut_lenth+int(i*real_lenth/5)),"\n")
            path_dot.append(SmallDot(path0.get_all_points()[int(cut_lenth+int(i*real_lenth/5))]))
            path_dot_index.append(int(i*len(path0.get_all_points())/5))
            slope_list.append(
                (Dot(path0.get_all_points()[path_dot_index[i]+1]).get_y()-
                              Dot(path0.get_all_points()[path_dot_index[i]]).get_y())
                /(Dot(path0.get_all_points()[path_dot_index[i]+1]).get_x()-
                              Dot(path0.get_all_points()[path_dot_index[i]]).get_x())
            )
            #
            print(f"第{i}个点斜率:",Dot(path0.get_all_points()[path_dot_index[i]+1]).get_y(),
                  Dot(path0.get_all_points()[path_dot_index[i]]).get_y()
                  ,Dot(path0.get_all_points()[path_dot_index[i]+1]).get_y()-
                              Dot(path0.get_all_points()[path_dot_index[i]]).get_y(),f"\n斜率:",slope_list[i])
            #
            path_dot_group=VGroup(path_dot_group,path_dot[i])
        self.play(Write(path_dot_group))


        #####


        num_line = NumberLine(x_range=[0, 2, 0.4], include_tip=True).set_width(camera.get_width()).scale(0.2)
        num_line.next_to(camera.get_edge_center(UP), DOWN)
        dot_Taxis=SmallDot()
        Delta_T=ValueTracker(0.1)
        tria_t = Triangle().set_width(camera.get_width()).scale(0.01).rotate(npy.pi).next_to(dot_Taxis,UP,buff=0)
        dot_Taxis.add_updater(lambda d:d.become(SmallDot(num_line.n2p(Delta_T.get_value()))))
        ...
        for i in range(5):
            dot_T.append(path_dot[i].copy().shift(RIGHT*Delta_T.get_value()))
            line_Delta_T.append(Line(dot_T[i],path_dot[i]))
            dot_and_dot.append(path_dot[i].copy().shift(RIGHT*Delta_T.get_value()+UP*Delta_T.get_value()*slope_list[i]))
            line_Delta_X.append(DashedLine(dot_and_dot[i],dot_T[i]))
            line_tangent.append(DashedLine(dot_T[i],path_dot[i]))
            dad_group=VGroup(dad_group,dot_and_dot[i])
            dt_group=VGroup(dt_group,dot_T[i])
            ldt_group=VGroup(ldt_group,line_Delta_T[i])
            ldx_group=VGroup(ldx_group,line_Delta_X[i])
            lt_group=VGroup(lt_group,line_tangent[i])

            dot_T[i].add_updater(lambda d:d.become(path_dot[i].shift(RIGHT*Delta_T.get_value())))
            line_Delta_T[i].add_updater(lambda d:d.become(Line(dot_T[i],path_dot[i])))
            dot_and_dot[i].add_updater(lambda d:d.become(path_dot[i].shift(RIGHT*Delta_T.get_value()+UP*Delta_T.get_value()*slope_list[i])))
            line_Delta_X[i].add_updater(lambda l:l.become(DashedLine(dot_and_dot[i],dot_T[i])))
            line_tangent[i].add_updater(lambda l:l.become(DashedLine(dot_T[i],path_dot[i])))
        self.play(Write(dt_group),Write(lt_group),Write(ldx_group),Write(dad_group),Write(ldt_group),Write(tria_t))
...
if __name__ == "__main__":
    system("manimgl 铜晶胞伸长_改.py View1 -p -l -n23")#-n23-n27-31