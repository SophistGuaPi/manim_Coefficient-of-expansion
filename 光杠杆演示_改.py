...
class View2(Scene):
    def construct(self):
        ...
        d0 = Dot()
        d1 = Dot().shift(RIGHT * 3)
        d2 = Dot().shift(LEFT * 5)
        d3 = Dot().shift(DOWN + RIGHT * 3)
        d4 = Dot().shift(LEFT * 5 + UP * 2)
        l0_0=Line(d0,d1)
        l0_1=Line(d0,d2)
        l1_0=DashedLine(d0,d3)
        l1_1=DashedLine(d0,d4)
        l0=VGroup(l0_0,l0_1)
        l1=VGroup(l1_0,l1_1)
        ...
        rect=Rectangle(height=0.2,width=8)
        triang=Triangle().scale(0.3)

        self.play(ShowCreation(rect))
        self.play(FadeIn(triang.next_to(d0,DOWN,0.1)))
        # self.play(triang.animate.next_to(d0,DOWN,0.1))
        self.play(Write(l2))
        self.play(ShowCreation(v0),Write(l0),Write(l1))
        self.play(Write(l3))
        triang.add_updater(lambda t: t.become(Triangle().scale(0.3)).next_to(d0, DOWN, 0.1))

        l1_0.add_updater(lambda l:l.become(DashedLine(d0,d3)))
        l1_1.add_updater(lambda l:l.become(DashedLine(d0,d4)))
        ...
        label0[1].set_opacity(0)
        label1[1].set_opacity(0)
        ...
        label2_0[1].set_opacity(0)
        label3_0[1].set_opacity(0)
        label2_0.arrange(UP)
        label3_0.arrange(DOWN)
        ...
        self.play(camera.animate.shift(UP * 2))
        ...
        # camera.capture(d0)
        self.play(d0.animate.shift(RIGHT*1.5),run_time=3)
...
if __name__ == "__main__":
    system("manimgl 光杠杆演示_改.py View3 -w")